...
type Page = 'home' | 'tool' | 'about' | 'contact'
...
const features = [
  {
    ...
    description: 'Applies attachment theory, John Gottman\'s research, and CBT principles to analyze communication.',
  },
  {
    icon: HeartPulse,
    title: 'Indian Relationship Context',
    description: 'Custom-trained on Indian relationship dynamics, cultural nuances, and communication styles.',
  },
  {
    icon: ShieldCheck,
    title: 'Clinical-Grade Accuracy',
    description: 'Powered by Pinecone RAG and Gemini AI for deep semantic analysis beyond surface-level sentiment.',
  },
]
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pt-20 pb-24 text-center">
          ...
          <div className="mb-2">
            <span className="inline-block rounded-full border border-border/80 bg-card px-4 py-1 text-xs font-medium uppercase tracking-widest text-muted-foreground shadow-sm">
              Team Warlords &nbsp;·&nbsp; Hackathon Project
            </span>
          </div>
          ...
          <p className="mt-6 mx-auto max-w-2xl text-lg sm:text-xl text-muted-foreground leading-relaxed text-pretty">
            ...
            psychology frameworks to diagnose relationship health — custom-trained for{' '}
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {features.map(({ icon: Icon, title, description }) => (
              <div key={title} className="flex flex-col gap-3 p-6 rounded-xl bg-background border border-border/60">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative">
            ...
            {steps.map(({ number, icon: Icon, title, description }) => (
              <div key={number} className="flex flex-col items-center text-center gap-4 p-8 rounded-2xl bg-card border border-border/60 hover:border-accent/40 hover:shadow-sm transition-all">
